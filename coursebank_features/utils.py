...
def export_certificate_data(email_address=None):
    tnow = datetime.now().strftime('%Y-%m-%dT%H:%M:%S.000Z')
    certificates = GeneratedCertificate.objects.all()

    fields = ['id', 'course_id', 'mode', 'name', 'email' , 'grade', 'created_date']
    ...
    with open(output_file, 'w', newline='') as csvfile:
        ...
        for certificate in certificates:
            user = User.objects.get(id=certificate.user_id)
            data = {
                'id': certificate.id,
        ...
        if email_address:
            email = EmailMessage(
                'Coursebank - Generated Certificates',
                'Attached file of Learner Certificates (as of {})'.format(tnow),
                'no-reply-learner-profiles@coursebank.ph',
                [email_address,],
                )
